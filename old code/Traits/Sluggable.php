...
trait Sluggable
{
    ...
    protected function getSluggableUniqueAttributeValue($name, $value)
    {
        ...
        // Base slug without counter
        $baseSlug = preg_replace('/' . preg_quote($separator) . '[0-9]+$/', '', trim($value));

        if (empty($baseSlug)) return rand(9999999,999999999);

        // Fetch all similar slugs from the DB in a single query
        $existingSlugs = $this->newQuery()
            ->where($name, 'LIKE', $baseSlug . '%')
            ->pluck($name)
            ->toArray();
